...
export default function Goals() {
  ...
  // Improved date handling for capsules
  const addCapsule = () => {
    if (!cTitle.trim() || !cOpen) return;
    const openDate = new Date(cOpen);
    // Set time to end of day (23:59:59) to ensure full day coverage
    openDate.setHours(23, 59, 59, 999);
  ...
  const openNow = (cid) => setCapsules(arr => arr.map(c => 
    c.id === cid ? { ...c, openedAt: new Date().toISOString() } : c
  ));
              ...
              return (
                <div className={`capsule ${unlocked ? "open" : "locked"}`} key={c.id}>
                    ...
                    <div className="cap-meta">
                      {unlocked ? (
                        <span className="pill open">
                          {c.openedAt ? `Opened on ${fmtDate(c.openedAt)}` : 'Available to open'}
                        </span>
                      ) : (
                        <span className="pill">Opens on {fmtDate(c.openDate)}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="cap-actions">
                    {unlocked && !c.openedAt && (
                      <button className="chip" onClick={()=>openNow(c.id)}>Open now</button>
                    )}
                    {!unlocked && (
                      <button className="chip" onClick={()=>openNow(c.id)}>Open early</button>
                    )}
